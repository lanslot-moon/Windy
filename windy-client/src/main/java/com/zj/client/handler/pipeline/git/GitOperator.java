...
public class GitOperator implements IGitProcessor {
    ...
    public Git pullCodeFromGit(GitMeta gitMeta, String branch, String workspace) throws Exception {
        ...
        // clone 仓库到指定目录
        return Git.cloneRepository().setURI(gitMeta.getGitUrl())
                .setDirectory(new File(workspace))
                .setCredentialsProvider(new UsernamePasswordCredentialsProvider(gitMeta.getTokenName(), gitMeta.getToken()))
                .setRemote(ORIGIN).setBranch(branch)
